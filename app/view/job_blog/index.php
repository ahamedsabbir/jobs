...
<?php
if(isset($get_5_post)){
	foreach($get_5_post as $key => $value){
?>
			    <div class="item mb-5">
				    <div class="media">
					    <img style="background-color:white; box-shadow: 1px 1px 2px #f99191, 0px 0px 25px #a2cda2, 0px 0px 7px #aeaed3;" class="mr-2 img-fluid post-thumb d-none d-md-flex" src="app/view/admin/upload/img/<?php echo $value["icon"]; ?>" alt="image">
					    <div class="media-body">
						    <h3 class="title mb-1"><?php echo $value["title"]; ?></h3>
                            <div class="meta mb-1">
								<span class="date"><?php echo format::formatDate($value["date"]); ?></span>
...
$next = $cunt+1;			
if($cunt != 1){
	echo "<a class='nav-link-prev nav-item nav-link rounded-left' href='".BASE_URL."job_blog_cntr/&page=".$prev."&start=".$prev_start."&item=".$item."'>Previous<i class='arrow-prev fas fa-long-arrow-alt-left'></i></a>";
}
if($next_start != $limit){
	echo "<a class='nav-link-next nav-item nav-link rounded-right' href='".BASE_URL."job_blog_cntr/&page=".$next."&start=".$next_start."&item=".$item."'>Next<i class='arrow-next fas fa-long-arrow-alt-right'></i></a>";
} 
echo "</nav>"
			?> 	
				</div>
